...
class _CustomMessageInputState extends State<CustomMessageInput> {
  ...
  void recordAndSendAudio() async {
    bool hasPermission = await FlutterAudioRecorder.hasPermissions;
    if (hasPermission) {
      var external = await path.getExternalStorageDirectory();

      File file = File((<String>[
        external.path,
        'audio${DateTime.now().millisecondsSinceEpoch}.aac'
      ].join("/")));
//      await file.create();
      var recorder = FlutterAudioRecorder(file.path,
